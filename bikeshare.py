...
def option(response, options = ('yes', 'no')):
    "Returns an input that is valid from the user(s) with an array of answers given."

    while True:
        option = input(response).lower().strip()
        ## terminates program if the input entered is end
        if option == 'end':
            raise SystemExit
...
def get_filters():
  ...
  ## TO DO: get user input for city (chicago, new york city, washington). HINT: Use a while loop to handle invalid inputs
    while True:
        city = option("\n which city(ies) will you like to select data from, Chicago, New York City, or Washington?\nEnsure to use commas when listing the city(ies)\n", CITY_DATA.keys())
        ## TO DO: get user input for month (all, january, february, ... , june)
        month = option("\n which month(s) from January to June, do you want to filter data from?\nEnsure to use commas when listing the months.\n>", months)
        ## TO DO: get user input for day of week (all, monday, tuesday, ... sunday) 
        day = option("\nwhich weekday(s) do you want do filter bikeshare data?\nEnsure to use commas when listing the weekday(s).\n>", weekdays)
...
def load_data(city, month, day):
    ...
    """
    print("\nThis Program is loading the data for filters of your choice.")
...
def station_stats(df):
    """Shows statistics of the most popular stations and trip."""
...
def user_stats(df, city):
    """Shows the statistics of bikeshare users."""
    ...
    try:
        ...
        print("No data of user genders for {}.".format(city.title()))
    ...
    try:
            ...
            print("Te youngest person to ride one bike was born in: " + most_recent_birth_year)
